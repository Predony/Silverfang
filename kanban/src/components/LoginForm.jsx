...
class LoginForm extends Component {
	...
	submitLoginForm() {
		...
		}).then(r => console.dir(r)).catch(err => console.error(err));
	...
	render() {
		return <form className="ClassLoginForm" ref={f => this.loginForm = f}>
			Login:
			<select onChange={this.handleChange} {(e)=>this.setState({username: e.target.value })} className='ClassUserName' value={this.state.username}>
      ...
      /> */
			...
			<input className='ClassPassword' type='password' value={this.state.password} onChange {(e)=>this.setState({password: e.target.value })}/>
